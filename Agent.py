...
class Agent:
	def __init__(self, n_turbines, agent_id):
		...
		turbine_observation_space = {
			'online_bool': Tuple((Discrete(2, start=0) for _ in range(N_PREVIOUS_TIME_STEPS[agent_id] + 1))),
			'power': Box(low=0, high=np.infty, shape=()),
			'wind_speed': Tuple((Box(low=0, high=np.infty, shape=()) for _ in range(N_PREVIOUS_TIME_STEPS[agent_id] + 1))),
			'wind_dir': Tuple((Box(low=180, high=360, shape=(), dtype=int) for _ in range(N_PREVIOUS_TIME_STEPS[agent_id] + 1))),
			'yaw_angle': Tuple((Box(low=YAW_LIMITS[0], high=YAW_LIMITS[1], shape=()) for _ in range(N_PREVIOUS_TIME_STEPS[agent_id] + 1))),
			'yaw_travel': Box(low=0, high=np.infty, shape=(), dtype=int),
			'ai_factor': Tuple((Box(low=AI_FACTOR_LIMITS[0], high=AI_FACTOR_LIMITS[1], shape=()) for _ in range(N_PREVIOUS_TIME_STEPS[agent_id] + 1))),
		...
		self.observation_space = Dict(
			{
				"power_ref_preview": Tuple((Box(low=0, high=np.infty, shape=())) for _ in range(N_PREVIEW_TIME_STEPS[agent_id] + 1)),
				"turbines": Dict({str(k): turbine_observation_space for k in self._turbine_ids})
			}
		)
		self.n_observations = ((N_PREVIEW_TIME_STEPS[agent_id] + 1) + # power_ref_preview
			self.n_turbines * (
				(N_PREVIOUS_TIME_STEPS[agent_id] + 1) # online-bool
				+ 1 # power
				+ (N_PREVIOUS_TIME_STEPS[agent_id] + 1) # wind_speed
				+ (N_PREVIOUS_TIME_STEPS[agent_id] + 1) # wind_dir
				+ (N_PREVIOUS_TIME_STEPS[agent_id] + 1) # yaw_angle_change
				+ 1 # yaw_travel
				+ (N_PREVIOUS_TIME_STEPS[agent_id] + 1) # ai_factor
				+ 1 # rotor_thrust
			))
