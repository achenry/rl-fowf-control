...
from ray.rllib.env.multi_agent_env import MultiAgentEnv, ENV_STATE, MultiAgentDict, MultiEnvDict
...
class FOWFEnv(MultiAgentEnv):
	...
	def __init__(self, config: EnvContext):
		...
		self.floris_input_file = config[
			'floris_input_file'] if 'floris_input_file' in config else "./9turb_floris_input.json"
		...
		self.agent_observation_space = Dict({'obs':
			                                     Dict({'layout_x': Box(low=min(coord.x1 for coord in
			                                                                   self.wind_farm.floris.farm.turbine_map.coords),
			                                                           high=max(coord.x1 for coord in
			                                                                    self.wind_farm.floris.farm.turbine_map.coords),
			                                                           shape=(self.n_turbines,), dtype=np.float16),
			                                           'layout_y': Box(low=min(coord.x2 for coord in
			                                                                   self.wind_farm.floris.farm.turbine_map.coords),
			                                                           high=max(coord.x2 for coord in
			                                                                    self.wind_farm.floris.farm.turbine_map.coords),
			                                                           shape=(self.n_turbines,), dtype=np.float16),
		...
		self.observation_space = Dict({k: self.agent_observation_space for k in self._agent_ids})
		
		# self.ax_ind_factor_indices = np.arange(0, self.n_turbines)
		# self.yaw_angle_indices = np.arange(self.n_turbines, 2 * self.n_turbines)
		
		self.mean_layout = [(turbine_coords.x1, turbine_coords.x2) for turbine_coords in
		                    self.wind_farm.floris.farm.turbine_map.coords]
	...
	def reset(self):
		...
		self.mean_wind_dir = np.random.choice(np.arange(WIND_DIR_RANGE[0], WIND_DIR_RANGE[1], self.wind_dir_var))
		...
		new_layout[0, :] = np.clip(new_layout[0, :], self.agent_observation_space['obs']['layout_x'].low,
		                           self.agent_observation_space['obs']['layout_x'].high)
		new_layout[1, :] = np.clip(new_layout[1, :], self.agent_observation_space['obs']['layout_y'].low,
		                           self.agent_observation_space['obs']['layout_y'].high)
		
		init_online_bools = [np.random.choice([0, 1], p=[self.offline_probability, 1 - self.offline_probability])
		...
		init_yaw_angles, set_init_ax_ind_factors, effective_init_ax_ind_factors \
		...
		self.wind_farm.calculate_wake(yaw_angles=init_yaw_angles,
		                              axial_induction=effective_init_ax_ind_factors)
	...
	def get_action_values(self, action_dict, online_bools):
		ax_ind_factor_idx = [int(action_dict[k] // len(YAW_ANGLES)) for k in self._agent_ids]
		yaw_angle_idx = [int(action_dict[k] % len(YAW_ANGLES)) for k in self._agent_ids]
		yaw_angles = YAW_ANGLES[yaw_angle_idx]
		set_ax_ind_factors = AX_IND_FACTORS[ax_ind_factor_idx]
		effective_ax_ind_factors = [a if online_bool else EPS for a, online_bool in
		                            zip(set_ax_ind_factors, online_bools)]
	...
	def step(self, action_dict):
		...
		self.mean_wind_speed = np.clip(self.mean_wind_speed, WIND_SPEED_RANGE[0], WIND_SPEED_RANGE[1])
		...
		self.mean_wind_dir = self.mean_wind_dir \
		                     + np.random.choice([-self.wind_dir_var, 0, self.wind_dir_var],
		                                        p=[self.wind_change_probability / 2,
		                                           1 - self.wind_change_probability,
		                                           self.wind_change_probability / 2])
		self.mean_wind_dir = np.clip(self.mean_wind_dir, WIND_DIR_RANGE[0], WIND_DIR_RANGE[1])
		...
		new_yaw_angles, set_new_ax_ind_factors, effective_new_ax_ind_factors = self.get_action_values(action_dict,
		                                                                                              new_online_bools)
	...
	def _agent_obs(self, agent_idx, online_bools):
		...
		try:
			assert all(turbine.ai_set in AX_IND_FACTORS or turbine.ai_set == 0 for turbine in
					   self.wind_farm.floris.farm.turbines) and \
				   all(turbine.yaw_angle in YAW_ANGLES for turbine in self.wind_farm.floris.farm.turbines)
		except Exception as e:
			print(e)
		
		return {'obs': {'layout_x': [coords.x1 for coords in self.wind_farm.floris.farm.turbine_map.coords],
		                'layout_y': [coords.x2 for coords in self.wind_farm.floris.farm.turbine_map.coords],
		                'ax_ind_factors':
			                [np.where(AX_IND_FACTORS == ai)[0][0] for ai in self.current_ax_ind_factors],
		                'yaw_angles':
			                [np.where(YAW_ANGLES == gamma)[0][0] for gamma in self.current_yaw_angles],
...
class FOWFEnvWithGroupedAgents(MultiAgentEnv):
	...
	def __init__(self, env_config):
		...
		tuple_obs_space = Tuple(
			[env.agent_observation_space] * env.n_turbines
		)
		
		tuple_act_space = Tuple(
			[env.agent_action_space] * env.n_turbines
		)
	...
	def observation_space_sample(self, agent_ids: list = None) -> MultiEnvDict:
		...
		return {k: {kk: {'obs': sample[kk]} for kk in range(sample.__len__())} for k in self._agent_ids}