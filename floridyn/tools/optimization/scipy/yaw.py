...
class AiOptimization(Optimization):
    """
    AiOptimization is a subclass of :py:class:`floris.tools.optimization.scipy.Optimization` that is used to optimize the yaw angles of all turbines in a Floris Farm for a single set of inflow conditions using the SciPy optimize package.
    """
    ...
    def __init__(
        ...
        self.reinitialize_opt(
            minimum_ai_factor=minimum_ai_factor,
            maximum_ai_factor=maximum_ai_factor,
            x0=x0,
            bnds=bnds,
    ...
    def _optimize(self):
        ...
        opt_ai_factors = self.residual_plant.x
        
        return opt_ai_factors
    
    def _set_opt_bounds(self, minimum_ai_factor, maximum_ai_factor):
        self.bnds = [(minimum_ai_factor, maximum_ai_factor) for _ in range(self.nturbs)]
    ...
    def optimize(self, verbose=True):
        ...
        opt_ai_factors = self._optimize()
        
        if np.sum(opt_ai_factors) == 0 and verbose:
            print(
    ...
    def reinitialize_opt(
        self,
        minimum_ai_factor=None,
        maximum_ai_factor=None,
        x0=None,
        bnds=None,
        opt_method=None,
        opt_options=None,
        ...
        Args:
            ...
            unc_options (dictionary, optional): A dictionary containing values
                ...
                **unc_pmfs** is None and contains the following key-value pairs:
                ...
                'pdf_cutoff': 0.995} are used. Defaults to None.
        """
        if minimum_ai_factor is not None:
            self.minimum_ai_factor = minimum_ai_factor
        if maximum_ai_factor is not None:
            self.maximum_ai_factor = maximum_ai_factor
        if x0 is not None:
            self.x0 = x0
        else:
            self.x0 = [
        ...
        if bnds is not None:
            ...
            self._set_opt_bounds(self.minimum_ai_factor, self.maximum_ai_factor)
        ...
        if self.include_unc & (self.unc_pmfs is None):
            ...
            if self.unc_options["std_ai"] > 0:
                ai_bnd = int(
                    np.ceil(
                        norm.ppf(
                            self.unc_options["pdf_cutoff"],
                            scale=self.unc_options["std_ai"],
                        )
                        / self.unc_options["pmf_res"]
                    )
                )
                ai_unc = np.linspace(
                    -1 * ai_bnd * self.unc_options["pmf_res"],
                    ai_bnd * self.unc_options["pmf_res"],
                    2 * ai_bnd + 1,
                )
                ai_unc_pmf = norm.pdf(ai_unc, scale=self.unc_options["std_ai"])
                # normalize so sum = 1.0
                ai_unc_pmf = ai_unc_pmf / np.sum(ai_unc_pmf)
            else:
                ai_unc = np.zeros(1)
                ai_unc_pmf = np.ones(1)
            
            self.unc_pmfs = {
                "wd_unc": wd_unc,
                "wd_unc_pmf": wd_unc_pmf,
                "ai_unc": ai_unc,
                "ai_unc_pmf": ai_unc_pmf,
            }
    
    # Properties
    
    @property
    def minimum_ai_factor(self):
        """
        ...
        **Note**: This is a virtual property used to "get" or "set" a value.
        ...
        """
        return self._minimum_ai_factor
    
    @minimum_ai_factor.setter
    def minimum_ai_factor(self, value):
        if not hasattr(self, "maximum_ai_factor"):
            self._set_opt_bounds(value, 0.33)
        else:
            self._set_opt_bounds(value, self.maximum_ai_factor)
        self._minimum_ai_factor = value
    
    @property
    def maximum_ai_factor(self):
        """
        ...
        **Note**: This is a virtual property used to "get" or "set" a value.
        ...
        """
        return self._maximum_ai_factor
    
    @maximum_ai_factor.setter
    def maximum_ai_factor(self, value):
        if not hasattr(self, "minimum_ai_factor"):
            self._set_opt_bounds(0.0, value)
        else:
            self._set_opt_bounds(self.minimum_ai_factor, value)
        self._maximum_ai_factor = value
