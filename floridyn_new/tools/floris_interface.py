...
class FlorisInterface(LoggerBase):
    ...
    def __init__(self, input_file=None, input_dict=None):
        ...
        **input_file** or **input_dict** is required.
        ...
        self.wind_dir_change_turb = [False for _ in
                                     self.floris.farm.turbines]  # keeps track of when the wind direction shifts
        ...
        for i, turbine in enumerate(self.floris.farm.turbines):
            ...
            if not hasattr(turbine, 'wind_field_buffer'):
                turbine.wind_field_buffer = WindFieldBuffer(self.floris.farm.flow_field.wake.combination_function,
                                                            len(self.floris.farm.turbines), turbine.number)
    ...
    def copy(self):
        ...
        return FlorisInterface(input_dict=self.floris.as_dict()) #, het_map=self.het_map) # wake, farm, turbine
    
    def calculate_wake(
        self, yaw_angles=None, axial_induction=None, no_wake=False, points=None, track_n_upstream_wakes=False,
        sim_time=None
    ):
        """
        ...
        for i, turbine in enumerate(self.floris.farm.turbines):
            ...
            (wind_speed, turbine.send_wake) = turbine.wind_field_buffer.get_wind_speed(wind_speed, turbine.send_wake,
                                                                                       sim_time)
            
            (buffered_wind_direction, turbine.send_wake) = turbine.wind_field_buffer.get_wind_direction(wind_direction,
                                                                                                        None,
                                                                                                        turbine.send_wake,
                                                                                                        sim_time)
        ...
        if (self.wind_speed_change and sim_time is not None) or (self.wind_dir_change and sim_time is not None) or (
            sim_time is not None and any(angle_changes)) or sim_time == 0:
            ...
            self.floris.farm.flow_field.calculate_wake(look_ahead=True, sim_time=sim_time,
                                                       propagate_wind_speed=self.propagate_wind_speed,
        ...
        if sim_time is not None:
            ...
            self.floris.farm.set_yaw_angles(self.steady_yaw_angles)
            self.reinitialize_flow_field(wind_speed=self.steady_state_wind,
                                         wind_direction=self.steady_state_wind_direction, steady_state=True)
    ...
    def reinitialize_flow_field(
        ...
        if not isinstance(wind_speed, list) and wind_speed is not None and sim_time is not None:
            ...
            self.floris.farm.flow_field.propagate_wind_speeds(self.floris.farm.wind_map.input_speed, wind_speed,
                                                              sim_time, first_x=self.first_x)
        ...
        if not isinstance(wind_direction, list) and wind_direction is not None and sim_time is not None:
            ...
            self.floris.farm.flow_field.propagate_wind_directions(self.floris.farm.wind_map.turbine_wind_direction,
                                                                  wind_direction, sim_time)
            ...
            self.steady_yaw_angles = np.array(
                [turbine.yaw_angle - wind_dir_shift for turbine in self.floris.farm.turbines])
    ...
    def get_plane_of_points(
        ...
        elif normal_vector == "x":  # Rules of thumb for cut plane plane
            if x1_bounds is None:
                coords = self.floris.farm.flow_field.turbine_map.coords
                max_diameter = self.floris.farm.flow_field.max_diameter
                y = [coord.x2 for coord in coords]
        ...
        elif normal_vector == "y":  # Rules of thumb for y plane parallel with streamwise dir
            if x1_bounds is None:
                coords = self.floris.farm.flow_field.turbine_map.coords
                max_diameter = self.floris.farm.flow_field.max_diameter
                z = [coord.x3 for coord in coords]
                x1_bounds = (min(z) - 2 * max_diameter, max(z) + 2 * max_diameter)
            if x2_bounds is None:
                hub_height = self.floris.farm.flow_field.turbine_map.turbines[
                    0
                ].hub_height
        ...
        if len(self.yawed_coords) == 0:
            self.vis_flow_field.calculate_wake(points=points, sim_time=sim_time, visualize=True,
                                               propagate_wind_speed=self.propagate_wind_speed, first_x=self.first_x)
        ...
        for (i, coord) in self.yawed_coords:
            ...
            self.vis_flow_field.calculate_wake(points=points, sim_time=sim_time, visualize=True,
                                               propagate_wind_speed=self.propagate_wind_speed, first_x=coord.x1)
            points = None
